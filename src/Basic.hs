...
import Control.Concurrent.STM (STM, TQueue, TVar, atomically, isEmptyTQueue, newTQueue, newTVar, readTQueue, readTVar, writeTQueue, writeTVar)
...
meetInStudy logger id =
  logMsg logger $ T.pack $ "Elf " <> show id <> " meeting in the study"

deliverToys :: Logger -> Int -> IO ()
deliverToys logger id =
  logMsg logger $ T.pack $ "Reindeer " <> show id <> " delivering toys"

helper1 :: Group -> IO () -> IO ()
helper1 group doTask =
...
--
-- first version, before `choose` is abstracted out
--
